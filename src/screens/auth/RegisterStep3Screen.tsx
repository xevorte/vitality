...
import { View, TextInput, TouchableOpacity, SafeAreaView, ScrollView, Image, Platform, KeyboardAvoidingView } from 'react-native';
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} className="h-screen">
        ...
        <ScrollView contentContainerStyle={{ flexGrow: 1, padding: 24, paddingBottom: 16 }} showsVerticalScrollIndicator={false}>
          ...
          <View className="flex-row justify-between items-center mt-auto pt-8">
            ...
            <TouchableOpacity className="bg-primary disabled:bg-[#E2E7FF] px-6 py-3.5 rounded-full flex-row items-center shadow-sm" disabled={!registerData?.height_cm || !registerData?.weight_kg || !registerData?.age} onPress={() => NavigationServices.push(NAVIGATION_NAME.AUTH.registerStep4Screen, {})}>
